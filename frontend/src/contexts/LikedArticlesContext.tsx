...
interface LikedArticlesContextType {
    likedArticles: GrokArticle[];
    toggleLike: (article: GrokArticle) => void;
...
export function LikedArticlesProvider({ children }: { children: ReactNode }) {
    const [likedArticles, setLikedArticles] = useState<GrokArticle[]>(() => {
    ...
    const toggleLike = (article: GrokArticle) => {
